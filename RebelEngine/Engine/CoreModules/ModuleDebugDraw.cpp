...
#include "Utils/DebugDraw.h"     // Debug Draw API. Notice that we need the DEBUG_DRAW_IMPLEMENTATION macro here!
...
public:
    ...
    void drawPointList(const dd::DrawVertex * points, int count, bool depthEnabled) override {
        ...
        if (already) glEnable(GL_DEPTH_TEST);
        else glDisable(GL_DEPTH_TEST);
    ...
    void drawGlyphList(const dd::DrawVertex * glyphs, int count, dd::GlyphTextureHandle glyphTex) override {
        ...
        if(!already_blend) glEnable(GL_BLEND);
        ...
        if(!already_blend) glDisable(GL_BLEND);
    ...
    void setupShaderPrograms() {
        ...
        if (linePointProgram_MvpMatrixLocation < 0) {
            LOG(_ERROR, "Unable to get u_MvpMatrix uniform location!");
        }
        checkGLError(__FILE__, __LINE__);
    ...
    static void linkProgram(const GLuint program) {
        ...
        if (status == GL_FALSE) {
            ...
            LOG(_ERROR, ">>> Program linker errors: %s", strInfoLog);
        }
    }
